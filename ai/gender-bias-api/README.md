...
```bash
gender-bias-api$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
gender-bias-api$ sam local start-api
gender-bias-api$ curl http://localhost:3000/classify_digit
```
...
```bash
gender-bias-api$ sam logs -n InferenceFunction --stack-name gender-bias-api --tail
```
...
```bash
aws cloudformation delete-stack --stack-name gender-bias-api
```
